...
class CurrentOrderFragment : Fragment() {
    ...
    private fun loadOrders() {
        ...
        BackgroundTask.run(
            task = { OrderManager.getInstance().getOrders(appContext).filter { it.isCurrentOrder() } },
            onSuccess = { currentOrders -> showOrders(currentOrders) },
            onFailure = { showOrders(emptyList()) }
        )
    }

    private fun showOrders(currentOrders: List<Order>) {
        if (currentOrders.isEmpty()) {
            tvEmptyOrders.visibility = View.VISIBLE
            tvEmptyOrders.text = "No current orders"
